...
class QuestionController extends AbstractController
{
    ...
    public function show(Request $request, QuestionRepository $questionRepo, int $id, EntityManagerInterface $em): Response
    {
        ...
        if ($user) {
            ...
            if ($commentForm->isSubmitted() && $commentForm->isValid()) {
                ...
                $question->setNbOfResponse($question->getNbOfResponse() + 1);
                $em->persist($comment);
                $em->flush();
    ...
    public function rating(Request $request, Question $question, VoteRepository $voteRepo, int $score, EntityManagerInterface $em)
    {
        $user = $this->getUser();
        ...
        if ($user !== $question->getAuthor()) {
            ...
            if ($vote) {
                if ($vote->isIsLike() && $score > 0 || (!$vote->isIsLike() && $score < 0)) {
                    $em->remove($vote);
                    $question->setRating($question->getRating() + ($score > 0 ? -1 : 1));
                } else {
                    $vote->setIsLike(!$vote->isIsLike());
                ...
                $vote->setIsLike($score > 0 ? true : false);
